...
function activate(context) {
  ...
  context.subscriptions.push(vscode.commands.registerTextEditorCommand('console.log.wrap', (editor, edit) => handle(Wrap.Inline)), vscode.commands.registerTextEditorCommand('console.log.wrap.string', (editor, edit) => handle(Wrap.Inline, false, false, FormatAs.String)), vscode.commands.registerTextEditorCommand('console.log.wrap.string.up', (editor, edit) => handle(Wrap.Up, false, false, FormatAs.String)), vscode.commands.registerTextEditorCommand('console.log.wrap.string.down', (editor, edit) => handle(Wrap.Down, false, false, FormatAs.String)), vscode.commands.registerTextEditorCommand('console.log.wrap.prefix', (editor, edit) => handle(Wrap.Inline, true)), vscode.commands.registerTextEditorCommand('console.log.wrap.input', (editor, edit) => handle(Wrap.Inline, true, true)), vscode.commands.registerTextEditorCommand('console.log.wrap.up', (editor, edit) => handle(Wrap.Up)), vscode.commands.registerTextEditorCommand('console.log.wrap.up.prefix', (editor, edit) => handle(Wrap.Up, true)), vscode.commands.registerTextEditorCommand('console.log.wrap.up.input', (editor, edit) => handle(Wrap.Up, true, true)), vscode.commands.registerTextEditorCommand('console.log.wrap.down', (editor, edit) => handle(Wrap.Down)), vscode.commands.registerTextEditorCommand('console.log.wrap.down.prefix', (editor, edit) => handle(Wrap.Down, true)), vscode.commands.registerTextEditorCommand('console.log.wrap.down.input', (editor, edit) => handle(Wrap.Down, true, true)));
}
exports.activate = activate;
function handle(target, prefix, input, formatAs) {
    ...
    if (ran == undefined) {
      ...
      if (prefix || getSetting("alwaysUsePrefix")) {
        if (getSetting("alwaysInputBoxOnPrefix") == true || input) {
          ...
          // wrapData.txt = "console.log(`%c "  .concat(wrapData.item, " = ",  "${",  wrapData.item, "}",  "`, 'color: yellow');")
          wrapData.txt = "\nconsole.table('%c "  .concat(wrapData.item, " = ...", "', 'color: lime');")  // numlock (chrome)
          wrapData.txt += "\n" + "console.table(".concat(wrapData.item) + ");\n";
          resolve(wrapData)
        }
      }
      else {
        switch (formatAs) {
          case FormatAs.String:
            ...
            // wrapData.txt = "console.log(".concat("'\\x1b[32m', '", wrapData.item, " = ', ", wrapData.item, ", '\\x1b[0m')")  // pausebreak (node)
            wrapData.txt = "console.log(".concat("'\\x1b[32m', '", wrapData.item, " = ', ", wrapData.item, ", '\\x1b[0m')")  // pausebreak (node)
            break;
            default:
              // wrapData.txt = wrapData.txt.replace('$txt', "".concat( '`%c ', item, "`, ", "'color: orange'"))
              // wrapData.txt = "  console.table('%c ".concat(wrapData.item, " = ", "',", " ", wrapData.item, ", 'color: orange');")  // scrollock (chrome)
              wrapData.txt = "console.table('%c ".concat(wrapData.item, " = ", "',", " 'color: gold', ", wrapData.item, ");")  // scrollock (chrome)
              break;
        }
        resolve(wrapData);
      }
      ...
      case Wrap.Inline:
        {
          currentEditor.edit(function (e) {
            ...
            currentEditor.selection = new vscode.Selection(new vscode.Position(wrap.ran.start.line, wrap.txt.length + wrap.ran.start.character), new vscode.Position(wrap.ran.start.line, wrap.txt.length + wrap.ran.start.character));
          });
        }
        break;
      case Wrap.Up:
...
var FormatAs;
(function (FormatAs) {
  FormatAs[FormatAs["String"] = 0] = "String";
})(FormatAs || (FormatAs = {}));
var Wrap;
(function (Wrap) {
  Wrap[Wrap["Inline"] = 0] = "Inline";
  Wrap[Wrap["Down"] = 1] = "Down";
  Wrap[Wrap["Up"] = 2] = "Up";
})(Wrap || (Wrap = {}));
function deactivate() {
  return undefined;
